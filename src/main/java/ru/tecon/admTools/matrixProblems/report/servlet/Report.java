 ...
 * post запрос
 * параметры в теле в виде: {"date":"08.10.2020","filterID":0,"filterValue":"текон","structID":1508}
 */
        ...
        try (Jsonb json = JsonbBuilder.create();
            ...
            try {
                ...
                for (Future<Boolean> future: futures) {
                    if (!future.get(10, TimeUnit.MINUTES)) {
                        logger.warning("Error create excel pages");
                        resp.sendError(500);
                    }
                }
            ...
            resp.setHeader("Content-Disposition", "attachment; filename=\"" +
                    URLEncoder.encode("Анализ", StandardCharsets.UTF_8) + " " +
                    URLEncoder.encode("первичных", StandardCharsets.UTF_8) + " " +
                    URLEncoder.encode("измерений.xlsx", StandardCharsets.UTF_8) + "\"");
